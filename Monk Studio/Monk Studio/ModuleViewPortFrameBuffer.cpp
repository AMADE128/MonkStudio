...
bool ModuleViewportFrameBuffer::Start() {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, App->window->width, appExternal->window->height, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
	...
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, appExternal->window->width, appExternal->window->height);
...
bool ModuleViewportFrameBuffer::CleanUp() {
	...
	texture ? glDeleteTextures(1, &texture) : (void)0;
	frameBuffer ? glDeleteFramebuffers(1, &frameBuffer) : (void)0;
	renderBufferoutput ? glDeleteRenderbuffers(1, &renderBufferoutput) : (void)0;
